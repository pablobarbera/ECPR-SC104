...
Well now turn to an analyis of geolocated tweets. Use the itouchmap website to identify a set of coordinates that would include most of Europe. Then, open a connection to the Streaming API and connect one minute of Twitter data from within the bounding box defined by this set of coordinates.
...
Using the data you just downloaded, prepare a frequency table that shows what Europen countries were most active on Twitter during that period. You can use similar code to what we just used (see lines 102 to 106 of 01-twitter-streaming-data-colllection.Rmd), but change `map.where("state"...` to `map.where("world"...` .
